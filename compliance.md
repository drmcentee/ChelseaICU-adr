...
Pending
The project has been agreed as part of CW+ Horizon Fellowship
Agreed at divisional level 06/2023
## Training lead
Dr James McEntee
...
To reduce administrative time burden for clinicians, through automation
To prepare our knowledge base for AI integration
## Background
ICU is a complex environment with up to 18 doctors rotating every 3, 4, 6 or 12 months. At least half of these doctors will never have worked in ICU before. We also have a broad team of nurses, AHPs, therapists and other staff. To work most effectively as a team, we need to work from a shared understanding of standard practice. 
## Problem
Currently we have issues related to:
1. division of knowledge
2. access to knowledge
3. a framework for knowledge creation
### Division of knowledge
Knowledge in the ICU is divided, and not freely accessible
...
Only the current ICU website is easily accessible and regularly maintained. However, due to funding constraints, it can only hold information that can be presented without password protection e.g. how to guides for Cerner, approved at the regional level.
...
Much of the knowledge in ICU is also not suited to the formal trust guideline template e.g. a local induction timetable, and so exists only within a shared drive or email thread. 
The inaccessibility of knowledge leads to poor data storage and review practice. The diversity of formats (PDF, email, images, Word documnets) prevents intra-operability such that work must be duplicated and manually transcribed between formats.
### A lack of a portable knowledge framework
Most departments have a significant 'institutional knowledge' and local culture that is maintained through personal memory and conversations. The success of this is wholly reliant on staff retention and much of that knowledge is lost when rotational doctors leave. 
We currently have no solution to document this culture/knowledge or for new information to be widely communicated within the department. This leads to cultural inertia and resistance to new, improved practice.
	It's easy to think of projects and quality improvement work that stops when a staff member leaves
With no framework to capture this knowledge, it is usually lost or locked in a non-transferrable format e.g. a presentation saved somewhere on a shared drive. This lack of 'portability' prevents conversion from institutional knowledge  to formally agreed practice. 
...
Containerised deployment via a cloud hosted virtual machine (VM) will allow this project to be piloted. DigitalOcean has been chosen as it provides a balance of operability and cost. It has become an industry standard, entrypoint for cloud service.
...
These costs could be reduced significantlly (sysadmin fees aside) if the system were trust hosted.

Within the structure of the project, there is scope for teaching resources to be offered out to external candidates for a fee. For example, 20 candidates paying £30 each would cover the project with overheads for expansion and card processing. This is well within the scope of the FOCC course alone.
...
Cost previously - 56hrs at £100/hr = £5600
Current cost - 10.5hrs at £100hr + £200/yr = £1250
...
### Transferrable
Since this project is open-source, fully documented, largely containerised and significantly built on text based deployment, it can be rolled out cross site and across network with minimal change.
...
The automation allowed by this project means alerts for review can be set 
...
However, to the project embeds within the department culture, a roll out plan is needed
...
Currently leads for all the issues raised within the Architecture Decision Records have been identified.
## Downtime plan
Since backup services are available within the plan
...
However, a more realistic scenario is of high server load causing issues. As such Kubernetes or other scalable architecture needs to be considered.
